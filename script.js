            ...
            if (user) {
                ...
                setTimeout(async () => {
                    ...
                    if (appInitialized && isAdmin(user)) {
                        ...
                        // If user is on admin page but not admin, redirect them to home
    ...
    eventsData = [
        {
            ...
            video: {
                type: 'youtube',
                url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', // Rick Roll for demo
                width: '100%',
                height: 'auto',
                fit: 'contain'
            },
 ...
 * @description Extracts the video ID from various YouTube URL formats and returns a standard embed URL.
 * @param {string} url The YouTube URL to normalize.
 * @returns {string} A standard YouTube embed URL (e.g., https://www.youtube.com/embed/VIDEO_ID) or an empty string if no valid ID is found.
 */
...
function renderEventPage(slug) {
    ...
    const videoWidth = event.video?.width || '100%'; // Default to 100% width
    const videoHeight = event.video?.height || 'auto'; // Default to auto height
            ...
            <div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-8 event-card">
                ...
                <div class="relative w-full overflow-hidden bg-black rounded-lg mb-8" style="width: ${videoWidth}; height: ${videoHeight}; max-width: 100%; margin: 0 auto;">
                    ${event.video.type === 'youtube'
                        ...
                        ? `<iframe width="${videoWidth}" height="${videoHeight}" class="absolute top-0 left-0 w-full h-full" src="${normalizeYouTubeUrl(event.video.url) || 'https://www.youtube.com/embed/dQw4w9WgXcQ'}?autoplay=1&mute=1&rel=0&playsinline=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`
                        // Generic video tag for other video types (MP4, HLS, etc.)
                      : event.video.url ? `<video width="${videoWidth}" height="${videoHeight}" class="absolute top-0 left-0 w-full h-full object-${event.video.fit || 'contain'}" controls autoplay muted playsinline>
                            <source src="${event.video.url}" type="video/mp4">
                            Your browser does not support the video tag.
                           </video>` : `<p class="text-gray-400 text-center">No video URL provided for this event type.</p>`
                    }
                </div>
...
function renderAdminPanel() {
    ...
    appContainer.innerHTML = `
        <div class="max-w-7xl mx-auto py-8 px-4">
            ...
            <nav class="bg-gray-800 p-4 rounded-lg shadow-md mb-8 flex flex-wrap justify-center gap-4">
                ...
                <button id="admin-nav-security" class="px-6 py-3 button-style text-white bg-orange-600 hover:bg-orange-700">
                    <i data-lucide="lock" class="w-5 h-5 mr-2"></i> Security
                </button>
    ...
    document.getElementById('admin-nav-security')?.addEventListener('click', renderSecuritySettings); // New security settings
 ...
 * @description Renders the "Security Settings" section in the admin panel, allowing
 * admins to reset their username and password.
 */
function renderSecuritySettings() {
    const adminContent = document.getElementById('admin-content');
    if (!adminContent) { console.error("Admin content area not found."); return; }
    ...
    adminContent.innerHTML = `
        <h3 class="text-3xl font-bold mb-6 text-white flex items-center">
            <i data-lucide="lock" class="w-7 h-7 mr-3 text-orange-400"></i> Security Settings
        </h3>
        <div class="space-y-6">
            <p class="text-gray-300">
                This section allows you to reset the administrator username (email) and password.
                <strong class="text-red-400">Use with caution!</strong> If you forget these, you may lose access to the admin panel.
            </p>
            <button id="reset-admin-credentials-button" class="px-6 py-3 button-style text-white bg-red-600 hover:bg-red-700">
                <i data-lucide="key-round" class="w-5 h-5 mr-2"></i> Reset Admin Username & Password
            </button>
        </div>
    `;
    if (typeof lucide !== 'undefined') {
    ...
    document.getElementById('reset-admin-credentials-button')?.addEventListener('click', handleResetAdminCredentials);
}

/**
 * @function handleResetAdminCredentials
 * @description Prompts the admin to reset their username and password.
 * This will update the locally stored `adminUsername` and `adminPassword` in `appSettings`.
 * IMPORTANT: This does NOT change the Firebase Authentication user's email/password.
 * It only changes the credentials used for the custom `isAdmin` check.
 */
async function handleResetAdminCredentials() {
    const confirmed = await showMessageBox(
        "Are you sure you want to reset the admin username and password? You will need to re-login with the new credentials.",
        true
    );

    if (confirmed) {
        const newUsername = prompt("Enter new admin username (email):");
        const newPassword = prompt("Enter new admin password:");

        if (newUsername && newPassword) {
            // Update the local appSettings object
            appSettings.adminUsername = newUsername.trim();
            appSettings.adminPassword = newPassword.trim();

            // Save the updated appSettings (excluding admin credentials from Firestore)
            await saveAppSettings();

            // Inform the user and force a logout to apply new credentials
            await showMessageBox("Admin credentials reset successfully! Please log in with your new username and password.");
            await window.signOut(auth); // Force logout
            window.location.hash = ''; // Redirect to home
            window.location.reload(); // Force full reload
        } else {
            await showMessageBox("Reset cancelled or invalid input provided.");
        }
    }
}

...
let galleryPrevButton;
let galleryNextButton;
...
function loadMockData() {
    ...
    eventsData = [
        ...
        },
        {
            ...
            chatEnabled: false, // Chat disabled for this mock event
            status: 'published',
            featured: false,
            likes: 78,
            views: 300,
 ...
 * Also handles unsubscription of previous chat listeners.
 */
function handleRouting() {
    // Unsubscribe from any previously active chat listener to prevent memory leaks/multiple listeners
    if (currentEventChatListener) {
        currentEventChatListener(); // Call the unsubscribe function
        currentEventChatListener = null; // Clear the reference
    }
...
function renderEventPage(slug) {
        ...
        <div class="max-w-7xl mx-auto py-8 px-4">
            ...
            <div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-8 event-card">
                ...
                <p class="text-gray-400 text-center mb-6 text-lg">
                    <i data-lucide="calendar" class="inline-block w-5 h-5 mr-2 align-middle"></i> ${new Date(event.dateTime).toLocaleString()}
                    <span class="mx-4 text-gray-600">|</span>
                    <i data-lucide="map-pin" class="inline-block w-5 h-5 mr-2 align-middle"></i> ${event.venue}
                </p>

                <!-- Video Player Section -->
                <div class="relative w-full pb-[56.25%] bg-black rounded-lg overflow-hidden mb-8">
                    ${event.video.type === 'youtube'
                        // YouTube embed iframe
                        ? `<iframe class="absolute top-0 left-0 w-full h-full" src="${event.video.url}?autoplay=1&rel=0" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`
                        // Generic video tag for other video types (MP4, HLS, etc.)
                        : `<video class="absolute top-0 left-0 w-full h-full object-contain" controls autoplay loop>
                ...
                <!-- Interactions: Only Share Button as requested (Like/Views removed from public view) -->
                ...
                <!-- Chat Section (Optional): Live chat for the event, displayed only if enabled by admin -->
                ${event.chatEnabled ? `
                    <div class="bg-gray-850 rounded-lg shadow-inner p-6">
                        <h3 class="text-3xl font-bold mb-6 text-white text-center flex items-center justify-center">
                            <i data-lucide="message-circle" class="w-7 h-7 mr-3 text-emerald-400"></i> Live Chat
                        </h3>
                        <div id="chat-messages" class="h-80 overflow-y-auto custom-scrollbar p-3 rounded-lg bg-gray-900 mb-4">
                            <!-- Chat messages will be loaded here by setupChatListener -->
                            <p class="text-gray-500 text-center py-4">Loading messages...</p>
                        </div>
                        <form id="chat-form" class="flex flex-col sm:flex-row gap-3">
                            <input type="text" id="chat-name-input" placeholder="Your Name" class="p-3 rounded-lg bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 sm:w-1/4" required>
                            <input type="text" id="chat-message-input" placeholder="Type your message..." class="flex-1 p-3 rounded-lg bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                            <button type="submit" class="px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-md button-style">
                                Send
                            </button>
                        </form>
                    </div>
    ...
    if (event.chatEnabled) {
        document.getElementById('chat-form')?.addEventListener('submit', (e) => handleChatMessageSubmit(e, event.id));
        setupChatListener(event.id); // Initialize real-time chat listener
    }
 ...
 */
function openGallery(startIndex, eventId) {
    // Ensure gallery elements are defined
    // These are now globally declared and assigned in DOMContentLoaded
    if (!galleryImageDisplay || !galleryPrevButton || !galleryNextButton || !galleryModal) {
    ...
    // Find the event object to get its gallery images
    currentEventForGallery = eventsData.find(e => e.id === eventId);
    if (!currentEventForGallery || !currentEventForGallery.gallery || currentEventForGallery.gallery.length === 0) {
        showMessageBox("No gallery images available for this event.");
        return;
    }
    currentGalleryImages = currentEventForGallery.gallery; // Set the global gallery images array
    currentGalleryIndex = startIndex; // Set the starting index
 ...
 * @description Updates the image source and visibility of navigation buttons in the fullscreen gallery modal.
 */
function updateGalleryDisplay() {
    // These are now globally declared and assigned in DOMContentLoaded
    if (!galleryImageDisplay || !galleryPrevButton || !galleryNextButton) {
    ...
    if (currentGalleryImages.length > 0) {
        ...
        galleryPrevButton.style.display = currentGalleryIndex > 0 ? 'block' : 'none';
        galleryNextButton.style.display = currentGalleryIndex < currentGalleryImages.length - 1 ? 'block' : 'none';
    }
}

// Keyboard navigation for gallery (left/right arrows, Escape key)
document.addEventListener('keydown', (e) => {
    // These are now globally declared and assigned in DOMContentLoaded
    if (galleryModal && galleryModal.classList.contains('active')) { // Only respond if gallery modal is open
        if (e.key === 'ArrowLeft' && galleryPrevButton) {
            galleryPrevButton.click(); // Simulate click on previous button
        } else if (e.key === 'ArrowRight' && galleryNextButton) {
            galleryNextButton.click(); // Simulate click on next button
        } else if (e.key === 'Escape' && galleryCloseButton) {
 ...
 * @description Sets up a real-time listener for chat messages for a specific event.
 * Displays messages and ensures the chat box scrolls to the bottom.
 * @param {string} eventId The ID of the event to listen for chat messages.
 */
function setupChatListener(eventId) {
    if (!db) {
        console.warn("Firestore not initialized for chat. Chat will not function.");
        return;
    }
    ...
    const chatCollectionRef = window.collection(db, `chats/${eventId}/messages`);
    // Query messages, ordered by creation time to ensure correct order
    const q = window.query(chatCollectionRef, window.orderBy('createdAt', 'asc'));

    // Set up the real-time listener and store its unsubscribe function
    currentEventChatListener = window.onSnapshot(q, (snapshot) => {
        chatMessagesContainer.innerHTML = ''; // Clear existing messages before re-rendering
        snapshot.forEach(doc => {
            const msg = doc.data();
            const messageElement = document.createElement('div');
            messageElement.className = 'mb-2 flex justify-between items-center'; // Added flex for delete button
            ...
            if (auth.currentUser && !auth.currentUser.isAnonymous && auth.currentUser.uid === msg.userId) {
                messageContent += `
                    <button class="text-red-400 hover:text-red-600 text-xs ml-2 delete-chat-message-button" data-message-id="${doc.id}" data-event-id="${eventId}">
        ...
        // Attach delete message listeners after messages are rendered
        document.querySelectorAll('.delete-chat-message-button').forEach(button => {
            button.addEventListener('click', async (e) => {
                const messageId = e.currentTarget.dataset.messageId;
                const eventId = e.currentTarget.dataset.eventId;
                await deleteChatMessage(eventId, messageId);
            });
        });

        chatMessagesContainer.scrollTop = chatMessagesContainer.scrollHeight; // Scroll to bottom
        if (typeof lucide !== 'undefined') {
            lucide.createIcons(); // Re-render icons for delete buttons
        }
    }, (error) => {
        console.error("Error listening to chat messages:", error);
        chatMessagesContainer.innerHTML = `<p class="text-red-400 text-center py-4">Error loading chat messages. Please try again.</p>`;
    });
}

/**
 * @function handleChatMessageSubmit
 * @description Handles the submission of a new chat message.
 * Adds the message to Firestore and enforces a message limit (max 40 messages).
 * @param {Event} e The form submit event.
 * @param {string} eventId The ID of the event the message belongs to.
 */
async function handleChatMessageSubmit(e, eventId) {
    e.preventDefault(); // Prevent default form submission behavior

    if (!db) {
        await showMessageBox("Firestore not initialized. Cannot send message.");
        return;
    }
    ...
    if (!chatNameInput || !chatMessageInput) {
        console.error("Chat input elements not found.");
        await showMessageBox("Chat input fields not available. Cannot send message.");
        return;
    }
    ...
    if (!senderName || !messageText) {
        await showMessageBox("Please enter your name and message.");
        return;
    }
    ...
    try {
        ...
        await window.addDoc(chatCollectionRef, {
            ...
            userId: auth.currentUser ? auth.currentUser.uid : 'anonymous', // Record user ID for potential moderation
        });
        chatMessageInput.value = ''; // Clear the message input field

        // --- Automated Chat Message Deletion (Limit to 40 messages) ---
        const q = window.query(chatCollectionRef, window.orderBy('createdAt', 'asc')); // Order by oldest first
        const snapshot = await window.getDocs(q); // Get all messages

        if (snapshot.size > 40) {
            const batch = window.writeBatch(db); // Create a Firestore batch for multiple deletions
            // Get the documents to delete (the oldest ones beyond the limit)
            for (let i = 0; i < snapshot.size - 40; i++) {
                batch.delete(snapshot.docs[i].ref); // Add delete operation to the batch
            }
            await batch.commit(); // Execute the batch deletions
            console.log(`Deleted ${snapshot.size - 40} oldest chat messages for event ${eventId}.`);
        }
    } catch (error) {
        console.error("Error sending chat message or managing chat limit:", error);
        await showMessageBox("Failed to send message. Please try again.");
    }
}

/**
 * @function deleteChatMessage
 * @description Deletes a specific chat message from Firestore.
 * @param {string} eventId The ID of the event the message belongs to.
 * @param {string} messageId The ID of the message to delete.
 */
async function deleteChatMessage(eventId, messageId) {
    if (!db) {
        await showMessageBox("Firestore not initialized. Cannot delete message.");
        return;
    }
    // Optional: Add a confirmation dialog here if desired
    const confirmed = await showMessageBox("Are you sure you want to delete this message?", true);
    if (!confirmed) return;

    try {
        const messageRef = window.doc(db, `chats/${eventId}/messages`, messageId);
        await window.deleteDoc(messageRef);
        console.log(`Message ${messageId} deleted from event ${eventId} chat.`);
        // The onSnapshot listener will automatically update the UI
    } catch (error) {
        console.error("Error deleting chat message:", error);
        await showMessageBox("Failed to delete message. Please try again.");
    }
}
...
function renderAdminPanel() {
    ...
    document.getElementById('admin-logout')?.addEventListener('click', async () => {
        ...
        await showMessageBox("Logged out successfully.");
        window.location.hash = ''; // Redirect to the home page after logout
    });
...
function renderEventsManagement() {
        ...
        <div id="event-edit-form-container" class="hidden mt-8 p-6 bg-gray-700 rounded-lg shadow-inner custom-scrollbar">
            ...
            <form id="event-form" class="space-y-4">
                ...
                <div class="flex items-center mt-4">
                    ...
                    <label for="auto-poster-checkbox" class="text-gray-300">Enable Auto-Poster Generation (adds as first gallery image)</label>
                    <button type="button" id="generate-poster-button" class="ml-4 px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg button-style hidden">
                        Generate Now
                    </button>
                </div>
                ...
                <!-- Chat and Status Settings -->
                <h5 class="text-xl font-medium mt-6 mb-3 text-white">Chat & Status</h5>
                <div class="flex items-center mb-4">
                    <input type="checkbox" id="chat-enabled-checkbox" class="mr-2 h-5 w-5 text-purple-600 rounded focus:ring-purple-500">
                    <label for="chat-enabled-checkbox" class="text-gray-300">Enable Chat for this Event</label>
                </div>
                <div>
    ...
    // Auto-poster checkbox listener to show/hide the "Generate Now" button
    const autoPosterCheckbox = document.getElementById('auto-poster-checkbox');
    const generatePosterButton = document.getElementById('generate-poster-button');
    if (autoPosterCheckbox && generatePosterButton) {
        autoPosterCheckbox.addEventListener('change', (e) => {
            if (e.target.checked) {
                generatePosterButton.classList.remove('hidden');
            } else {
                generatePosterButton.classList.add('hidden');
            }
        });
    }

    // Event listener for the "Generate Now" poster button
    if (generatePosterButton) {
        generatePosterButton.addEventListener('click', async () => {
            ...
            if (!titleInput || !dateTimeInput || !venueInput || !slugInput) {
                await showMessageBox("Error: Missing event detail inputs for poster generation.");
                return;
            }
            ...
            if (!title || !dateTime || !venue || !slug) {
                await showMessageBox("Please fill in event Title, Date & Time, Venue, and URL Slug to simulate poster generation.");
                return;
            }

            const eventDate = new Date(dateTime).toLocaleDateString();
            const eventTime = new Date(dateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

            const posterUrl = `https://placehold.co/800x450/3b0764/ffffff?text=${encodeURIComponent(title)}%0A${eventDate}%20${eventTime}%0A${encodeURIComponent(venue)}%0ALink:%20${encodeURIComponent(window.location.origin + window.location.pathname + `#event=${slug}`)}`;

            const confirmed = await showMessageBox("A placeholder poster will be generated and added to the gallery. In a real app, this would be a server-side image generation. Proceed?", true);
            if (confirmed) {
                // Add the generated poster URL as the first gallery image
                const galleryImagesAdmin = document.getElementById('gallery-images-admin');
                if (galleryImagesAdmin) {
                    const newPosterField = createAdminGalleryImageField({
                        id: generateUniqueId(),
                        url: posterUrl,
                        caption: `Auto-generated Poster for ${title}`
                    });
                    // Insert at the beginning of the gallery list
                    galleryImagesAdmin.prepend(newPosterField);
                    applyTheme(); // Reapply theme for new elements
                    await showMessageBox("Placeholder poster added to gallery!");
                }
            }
        });
    }
...
function showEventEditForm(event = null) {
    ...
    const chatEnabledCheckbox = document.getElementById('chat-enabled-checkbox');
    ...
    if (!eventIdInput || !titleInput || !slugInput || !dateTimeInput || !venueInput || !videoTypeSelect || !videoUrlInput || !videoFitSelect || !thumbnailUrlInput || !thumbnailFileInput || !chatEnabledCheckbox || !eventStatusSelect || !eventFeaturedCheckbox || !eventBgColorInput || !eventBgImageUrlInput || !eventBgFileInput || !galleryImagesAdmin || !eventFormTitle || !slugPreview) {
    ...
    if (event) {
        ...
        chatEnabledCheckbox.checked = event.chatEnabled || false;
        ...
        chatEnabledCheckbox.checked = true; // Default chat enabled for new events
...
async function handleSaveEvent(e) {
    ...
    const chatEnabled = document.getElementById('chat-enabled-checkbox')?.checked;
    ...
    const eventObj = {
        ...
        venue,
        video: { type: videoType, url: videoUrl, fit: videoFit },
        thumbnailUrl,
        gallery,
        bg: eventBackground,
        client,
...
function renderThemeSettings() {
    ...
    adminContent.innerHTML = `
        ...
        <form id="theme-settings-form" class="space-y-6">
            ...
            <!-- 3D Style Options -->
            <div class="flex items-center space-x-4">
                <label class="block text-gray-300 text-lg font-medium">3D Style (Visual Hint):</label>
                <input type="radio" id="style-neumorphic" name="depth-style" value="neumorphic" class="mr-2 h-4 w-4 text-purple-600 focus:ring-purple-500" ${appSettings.theme.depthStyle === 'neumorphic' ? 'checked' : ''}>
                <label for="style-neumorphic" class="text-gray-300">Neumorphic</label>
                <input type="radio" id="style-glass" name="depth-style" value="glass" class="ml-4 mr-2 h-4 w-4 text-purple-600 focus:ring-purple-500" ${appSettings.theme.depthStyle === 'glass' ? 'checked' : ''}>
                <label for="style-glass" class="text-gray-300">Glassmorphism</label>
                 <input type="radio" id="style-flat" name="depth-style" value="flat" class="ml-4 mr-2 h-4 w-4 text-purple-600 focus:ring-purple-500" ${appSettings.theme.depthStyle === 'flat' ? 'checked' : ''}>
                <label for="style-flat" class="text-gray-300">Flat</label>
            </div>
             <p class="text-gray-400 text-sm mt-1">Note: Actual 3D effects are primarily controlled by CSS. This setting acts as a visual hint for administrators.</p>
